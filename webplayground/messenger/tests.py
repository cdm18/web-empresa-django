...
class ThreadTestCase (TestCase):
    ...
    def test_add_messages_to_thread (self):      
        self.thread.users.add (self.user1, self.user2)
        message1 = Message.objects.create (user = self.user1, content = 'Como estas')
        message2 = Message.objects.create (user = self.user2, content = 'Muy bien y tu?')
    ...
    def test_add_message_from_user_not_in_thread (self):
        ...
        message1 = Message.objects.create (user = self.user1, content = 'Como estas')
        message2 = Message.objects.create (user = self.user2, content = 'Muy bien y tu?')
        message3 = Message.objects.create (user = self.user3, content = 'Wassuuuuuup')
